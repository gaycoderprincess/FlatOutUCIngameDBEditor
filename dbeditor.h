...
void DBPropertyEditLoop(LiteDb* node, const char* propName, void* pCurrentPropertyEditing) {
	...
	if (type == DBVALUE_NODE) {
		...
		if (bak < LiteDb::gNodes || *(uintptr_t*)bak != nodeVtable) {
		...
		if (pCurrentPropertyEditingNodeTemp < LiteDb::gNodes || *(uintptr_t*)pCurrentPropertyEditingNodeTemp != nodeVtable) pCurrentPropertyEditingNodeTemp = bak;
